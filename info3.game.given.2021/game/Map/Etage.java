...
public class Etage {
	...
	Salle[] salles;	//Tableau des salle. salles[0] est l'entrée, et la dernière est la salle du boss
	
	int niveau;
	
	
	public Etage(int niv) {
	...
	void init_etage() {	//Créer l'entrée
		Random r = new Random();
		int j = r.nextInt(3) +1;
		salles[0] = new Salle(j, "entree");
	}
	
	void creer_salles() {	//créé toutes les salles et leur portes entre l'entrée et le boss
	...
	int nbr_de_portes_dispo(int indice) { //Renvoie le nombre de portes qu'il ne faut pas mettre à la salle pour pas faire boucler l'étage
	...
	void Ajout_salle_boss() {
		...
		while (salles[niveau+1].Porte_non_liees() == true) { //On supprime les portes de l'avant dernière salle si elles ne sont pas lié
