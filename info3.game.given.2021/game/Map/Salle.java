...
public class Salle {
	...
	void set_compo(String type) {
		...
		try
        {
            ...
            if (type == "normale") {	//Si la salle est normale on choisi un des deux pattern aléatoirement en sautant plus ou moins de ligne
            	Random x = new Random();
            	int j = x.nextInt(2);
            	for (int i = 0; i<2550*j; i++) {	//On saute 2550 caractères (nombre de caractère pour faire un pattern)si j=1
            		r=br.read();
            	}
            }
	...
	void Ajouter_portes(int nbr_de_portes) {	//Ajoute plusieurs portes
		for (int i = 0; i < nbr_de_portes; i++) {
			...
			if (v) {nbr_portes++;}	//SI l'ajout à réussi, on incrémente nbr_portes
		}
	}
